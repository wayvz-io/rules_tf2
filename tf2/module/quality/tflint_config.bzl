...
def tf_generate_tflint_config(name, provider_configurations = None, **kwargs):
    """Generate a TFLint configuration file.

    This is a macro wrapper around tf_tflint_config rule.

    Args:
        name: Target name
        provider_configurations: Provider configurations label
        **kwargs: Additional arguments passed to tf_tflint_config
    """
    tf_tflint_config(
        name = name,
