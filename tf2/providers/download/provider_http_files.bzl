...
def parse_terraform_lock_file(lock_content):
    ...
    Args:
        lock_content: Content of the terraform.lock.hcl file as a string
...
def create_provider_http_file_repositories(_, providers_info):
    """Create individual http_file repositories for each provider binary.

    Args:
        _: Unused module context parameter
    ...
    for _, provider_data in providers_info.items():
