...
load("//tf/core/rules:module.bzl", "tf_module_rule", "tf_module_deps")
# tf_stack_rule removed - functionality merged into tf_module
load("//tf/testing:validate.bzl", "tf_validate_test")
load("//tf/testing:format.bzl", "tf_format_test", "tf_format")
load("//tf/testing:lint.bzl", "tf_lint_test")
load("//tf/testing:versions.bzl", "tf_versions_check_test", "tf_generate_versions", "tf_generate_versions_from_mirrors")
load("//tf/testing:docs.bzl", "tf_doc_test", "tf_generate_docs")
load("//tf/testing:module_deps.bzl", "tf_module_deps_test")
load("//tf/testing:organization.bzl", "tf_organization_check_test", "tf_reorganize")
load("//tf/testing:lockfile.bzl", "tf_generate_lockfile_for_validation", "tf_no_lockfile_check_test")
load("//tf/testing:test.bzl", "tf_test")
load("//tf/publish/oci:oci_push.bzl", "oci_push", "tf_module_push_oci")
...
def tf_module(
    ...
    if test_files:
        ...
        # Determine the provider registry based on the workspace
        # Use unpacked providers for filesystem_mirror
        if native.repository_name() == "@rules_tf2" or native.package_name().startswith("tests/"):
            provider_registry = "@tf_providers_test//:unpacked_providers"
        else:
    ...
    if not skip_validation:
        # Automatically determine the provider registry based on the workspace
        # In tf2 module, use @tf_providers_test
        # In main workspace, use @tf_provider_registry
        # Use unpacked providers for filesystem_mirror
        if native.repository_name() == "@rules_tf2" or native.package_name().startswith("tests/"):
            provider_registry = "@tf_providers_test//:unpacked_providers"
        else:
