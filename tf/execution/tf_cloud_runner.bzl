...
def tf_cloud_configuration(
        ...
        **kwargs):
    """Macro to create Terraform Cloud runner targets.
    
    This creates targets:
    - {name}: Main runner target for arbitrary terraform commands
    - {name}_validate: Local validation (terraform validate)
    - {name}_tfc_plan: Runs terraform plan (equivalent to speculative plan)
    - {name}_tfc_apply: Runs terraform apply
    ...
    Example:
        tf_cloud_configuration(
            name = "core_aws_hub",
            module = "//iac/stacks/aws/hub",
            variables = ":lab_vars",
            workspace_name = "core-aws-hub",
            organization = "Wayvz",
        )
        
        # Then run:
        # bazel run //path:core_aws_hub_validate       # Local validation
        # bazel run //path:core_aws_hub_tfc_plan       # Terraform plan
        # bazel run //path:core_aws_hub_tfc_apply      # Terraform apply
        # bazel run //path:core_aws_hub -- state list  # Any terraform command
    """
